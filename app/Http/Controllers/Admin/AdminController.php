...
class AdminController extends Controller
{
    ...
    public function addNewsPost(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'newsImage' => $id ? 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048' : 'required|image|mimes:jpeg,png,jpg,gif|max:2048', // Validate image only if adding new post
        ]);
        ...
        $news = ($id) ? Posts::find($id) : new Posts();
        ...
        if ($id && $request->hasFile('newsImage')) {
    ...
    public function editNewsForm(Request $request)
    {
        ...
        $cat_id = $newsPost->category_id;
        $selectedTags = explode(',', $newsPost->tags);

        $newsCat = Categories::get()->where('parent_category', 0);

        $newsCatName = Categories::find($cat_id);

        $tags = Tags::all();

        return view("editNews", compact('newsCat', 'tags', 'newsPost', 'newsCatName', 'selectedTags'));
    }

    public function deletePost($id)
    {
        $deleteImage = Posts::find($id);
    ...
    public function viewPost(Request $request)
    {
        ...
        $catName = Categories::find($catId);
        return view('viewPost',compact('posts','catName'));
    }
}